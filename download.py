...
class Download(Client):
    ...
    def __init__(self, dir_path: str = None):
        ...
        # self.dir = dir_path if dir_path else dir_path  # 提供title2path方法，非法字符串由来源自行处理
        # print('目录')
        # print(self.dir)
        # self.__server()
    ...
    def download(self, url: str, filename: str = None, options=None):
        """
        添加下载任务
        单个任务特有参数此时设置
        ...
        if options is None:
            options = {}
        if filename:
            options['out'] = filename  # 提供title2path方法，非法字符串由来源自行处理
    ...
    def start_loop(self, on_loop):
        ...
        回调函数接收当前下载List
            ...
            '下载进度': 12.25%
        """
        threading.Thread(target=self.__on_loop, kwargs={'on_loop': on_loop}).start()

    def stop_loop(self):
        self.loop = False

    def __on_loop(self, on_loop):
        self.loop = True
        while self.loop:
            if callable(on_loop):
                status = []
